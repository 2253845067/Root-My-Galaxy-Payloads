...
static int argument_count(char *const arguments[])
{
    int count = 0;
...
{
    int argc = argument_count(arguments);

    if (!strcmp(arguments[0], "a536-probe"))
        return a536_probe_main();
    if (!strcmp(arguments[0], "a536-q0"))
        return a536_poc_main(argc, (char **)arguments);
    return a536_bridge_main(argc, (char **)arguments);
}
...
static int root_helper_id(const char *helper, char *proof, size_t proof_size)
{
    ...
    if (!child) {
        if (dup2(pair[1], STDOUT_FILENO) < 0 ||
            dup2(pair[1], STDERR_FILENO) < 0)
            _exit(126);
        close(pair[0]);
        close(pair[1]);
...
__attribute__((constructor)) static void a536_app_payload(void)
{
    static int started;
    ...
    FILE *stream;
    char *arguments[] = {
        "a536-bridge", "chain-ks-auto", "a536-q0", NULL,
    };
    char line[1024];
    char proof[512];
    char context[128];
    ...
    if (setenv("A536_KS_PROBE", "a536-probe", 1) ||
        setenv("A536_KS_SKB_PAYLOAD", "1", 1) ||
        setenv("A536_ORACLE_DETACH", "1", 1)) {
        fprintf(stderr, "A536_APP_FAIL setenv errno=%d %s\n", errno,
                strerror(errno));
        _exit(1);
    }
    if (pipe(pair)) {
    ...
    if (!chain) {
        ...
        close(pair[0]);
        close(pair[1]);
        _exit(a536_bridge_main(3, arguments));
    }
    close(pair[1]);
    ...
    printf("A536_APP_START uid=%d euid=%d pid=%d chain=%d context=%s\n",
           getuid(), geteuid(), getpid(), chain, context);
    while (fgets(line, sizeof(line), stream)) {
        ...
        if (!strncmp(line, "CHAIN_FAIL", 10) ||
            !strncmp(line, "ARW_FAIL", 8) ||
